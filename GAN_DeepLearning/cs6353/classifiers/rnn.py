...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Step 2: Transform input captions to word vectors using embeddings
        word_vectors, cache_embed = word_embedding_forward(captions_in, W_embed)

        # Use RNN to compute hidden states for all timesteps
        if self.cell_type == 'rnn':
            h, cache_rnn = rnn_forward(word_vectors, h0, Wx, Wh, b)

        # Step 4: Compute scores over the vocabulary using hidden states
        scores, cache_vocab = temporal_affine_forward(h, W_vocab, b_vocab)

        # Step 5: Compute loss using temporal softmax
        loss, dscores = temporal_softmax_loss(scores, captions_out, mask, verbose=False)

        # Backward pass
        # Step 4 (backward): Backprop through temporal affine layer
        dh, dW_vocab, db_vocab = temporal_affine_backward(dscores, cache_vocab)

        # Step 3 (backward): Backprop through RNN
        if self.cell_type == 'rnn':
            dword_vectors, dh0, dWx, dWh, db = rnn_backward(dh, cache_rnn)

        # Step 2 (backward): Backprop through word embedding layer
        dW_embed = word_embedding_backward(dword_vectors, cache_embed)

        # Step 1 (backward): Backprop through affine transformation for initial hidden state
        dfeatures = dh0 @ W_proj.T
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        # Compute the initial hidden state from image features
        h = features @ W_proj + b_proj

        # Start token for all samples
        current_word = self._start * np.ones((N,), dtype=np.int32)  # Shape (N,)
        ...
        for t in range(max_length):
            ...
            # Compute scores for the vocabulary
            scores = h @ W_vocab + b_vocab

            # Choose the word with the highest score and store it
